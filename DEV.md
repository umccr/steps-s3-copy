...
## Copy/Dry-run report preview (local)

The HTML reports are generated by `createHtmlReport()` and `createDryRunHtmlReport()` (in `packages/steps-s3-copy/lambda/summarise-copy-lambda/`) and are used by the summarise-copy Lambda (`summarise-copy-lambda/summarise-copy-lambda.ts`). To preview the reports locally (without running the full Step Functions workflow), run the dev preview scripts from `dev-testsuite/`:

```bash
bun run test-copy-report
bun run test-dryrun-report
```

These commands generate `dev_copy_report.html` and `dev_dryrun_report.html` in the `dev-testsuite/` directory, respectively, and then attempt to open them in your browser (best-effort).
